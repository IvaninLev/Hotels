...
@php
    ...
    if (isset($field['dependencies']) && is_array($field['dependencies'])){
        $field['element'] = $field['dependencies']['element'] ?? false;
        $field['data_url'] = $field['dependencies']['data_url'] ?? '';
    }
@endphp
...
<select
    ...
    data-depended-select="{{$field['element']}}"
    bp-field-main-input
...
@push('crud_fields_scripts')
    {{-- include select2 js --}}
    @basset('https://unpkg.com/select2@4.0.13/dist/js/select2.min.js')
    ...
    <script>
        ...
        $('#{{$field['name']}}').on('change.select2', function (event) {
            console.log(event)
            const citySelector = $('#{{$field['element']}}')
            if (citySelector) {
                $.ajax({
                    method: 'GET',
                    url: "{{ $field['data_url'] }}",
